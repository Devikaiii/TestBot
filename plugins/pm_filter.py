...
async def fil_mod(client, message): 
      ...
      try: 
         ...
         return await message.reply("On or Off")
      
      m = await message.reply("Processing...")

      if args in mode_on:
          FILTER_MODE[str(message.chat.id)] = "True" 
          await m.edit("Auto Filter Turned On.")
      
      elif args in mode_of:
          FILTER_MODE[str(message.chat.id)] = "False"
          await m.edit("Auto Filter Turned Off.")
      else:
          await m.edit("Use :- /autofilter on | /autofilter off")
...
async def next_page(bot, query):
    ...
    if not search:
        await query.answer("ഇപ്പഴാണോ വരുന്നേ. 🥲", show_alert=True)
        return
...
async def advantage_spoll_choker(bot, query):
    ...
    if not movies:
        return await query.answer("ഇപ്പഴാണോ വരുന്നേ 🥲", show_alert=True)
...
async def cb_handler(client: Client, query: CallbackQuery):
    if query.data == "close_data":
        ...
        if chat_type == "private":
            ...
            if grpid is not None:
                ...
                try:
                    ...
                    await query.message.edit_text("Please Make Sure I'm Present in Your Group!!", quote=True)
        ...
        if mkact:
            ...
            await query.message.edit_text('', parse_mode="md")
        return await query.answer('')
    ...
    if query.data.startswith("file"):
        ...
        try:
            if AUTH_CHANNEL and not await is_subscribed(client, query):
                ...
                await query.answer("Cʜᴇᴄᴋ PM. I'ᴠᴇ Sᴇɴᴅ Fɪʟᴇs Tᴏ Yᴏᴜʀ PM.", show_alert=True)
        ...
        if AUTH_CHANNEL and not await is_subscribed(client, query):
            await queryanswer("Nice Try.", show_alert=True)
            return
        ident, file_id = query.data.split("#")
        files_ = await get_file_details(file_id)
        if not files_:
    ...
    await removebg_stick
        ...
        reply_markup = InlineKeyboardMarkup(buttons)
        await query.message.edit_text(
            text=script.ONE_TXT.format(query.from_user.mention, temp.U_NAME, temp.B_NAME),
            disable_web_page_preview=True,
            reply_markup=reply_markup,
            parse_mode='html'
        )
        ...
        buttons = [[
            InlineKeyboardButton('« Back', callback_data='bio'),
        ...
        if settings is not None:
            buttons = [
                ...
                [
                    ...
                    InlineKeyboardButton('✅ ' if settings["botpm"] else '❌',
                                         callback_data=f'setgs#botpm#{settings["botpm"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('ʙᴀᴛᴄʜ',
                                         callback_data=f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}'),
                    InlineKeyboardButton('✅ ' if settings["file_secure"] else '❌',
                                         callback_data=f'setgs#file_secure#{settings["file_secure"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('ɪᴍᴅʙ', callback_data=f'setgs#imdb#{settings["imdb"]}#{str(grp_id)}'),
                    InlineKeyboardButton('✅' if settings["imdb"] else '❌',
                                         callback_data=f'setgs#imdb#{settings["imdb"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('sᴘᴇʟʟ ᴄʜᴇᴄᴋ',
                                         callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}'),
                    InlineKeyboardButton('✅' if settings["spell_check"] else '❌',
                                         callback_data=f'setgs#spell_check#{settings["spell_check"]}#{str(grp_id)}')
                ],
                [
                    InlineKeyboardButton('ᴡᴇʟᴄᴏᴍᴇ', callback_data=f'setgs#welcome#{settings["welcome"]}#{str(grp_id)}'),
                    InlineKeyboardButton('✅' if settings["welcome"] else '❌',
